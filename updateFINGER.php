<?php
  ...
  if (!empty($_POST)) {
    ...
    $sql = "UPDATE esp32_table_finger_update SET finger = ?, time = ?, date = ? WHERE id = ?";
    $q = $pdo->prepare($sql);
    $q->execute(array($finger,$tm,$dt,$id));
    Database::disconnect();
    //........................................ 
    ...
    while ($found_empty == false) {
      ...
      $sql = 'SELECT * FROM esp32_table_finger_record WHERE id="' . $id_key . '"';
      $q = $pdo->prepare($sql);
      $q->execute();
    ...
    $sql = "INSERT INTO esp32_table_finger_record (id,board,finger,time,date) values(?, ?, ?, ?, ?)";
		$q = $pdo->prepare($sql);
		//$q->execute(array($id_key,$board,$temperature,$humidity,$status_read_sensor_dht11,$led_01,$led_02,$tm,$dt));
    $q->execute(array($id_key,$board,$finger,$tm,$dt));
    //::::::::
